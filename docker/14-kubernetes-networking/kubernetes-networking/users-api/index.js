...
app.post('/signup', async (req, res) => {
  ...
  try {
    const hashedPW = await axios.get(
      `${process.env.AUTH_URL}/hashed-password/` + password
    );
...
app.post('/login', async (req, res) => {
  ...
  //   `${process.env.AUTH_URL}/token/` + hashedPassword + '/' + password
  // );

  // Automatically generated and configured IP address by kubernetes itself
  const response = await axios.get(
    `http://${process.env.AUTH_SERVICE_SERVICE_HOST}:3000/token/` +
      hashedPassword +
      '/' +
      password
  );
